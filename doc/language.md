...
| `${name[key]}` | Associative-array value for `key` (`typeset -A`); unset key → empty |
| `${(k)name}` / `${(v)name}` | Associative-array keys / values (key order); one field each; empty → no fields |
