...
| **expand** | `src/expand/` | `$` / backticks / field splitting pieces → `ExpandedWord` |
| **glob** | `src/glob/` | Pathname expansion (`* ? […]`); walk + bracket matching |
...
| **env** | `src/env/` | `ShellEnvironment`: vars, locals, aliases, argv, history, keys, dirstack, jobs |
